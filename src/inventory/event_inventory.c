...
static void check_object(game_t *g, sfVector2i pos, sfVector2u size)
{
    ...
    for (int i = 0; i < NB_OBJ; i++) {
        ...
        if (Mouse_Pressed(sfMouseLeft) && g->t.sec - tmp > 0.2 &&
        sfIntRect_contains(&(sfIntRect){object.body.pos.x - 64, object.body.pos
        .y, 64, 64}, pos.x * (1920.0 / size.x), pos.y * (1080.0 / size.y))) {
