...
button_t *init_settings_button(sfRenderWindow *window, int nb)
{
    ...
    buttons[0] = create_button(9, (sfVector2f){size.x * 0.15, size.y * 0.60},
    create_button_text(50, (sfVector2f){size.x * 0.15 + 130, size.y *
    0.60 + 20}, "FPS : 60"), &manage_framerate);
    buttons[1] = create_button(9, (sfVector2f){size.x * 0.60, size.y *
    0.60}, create_button_text(40, (sfVector2f){size.x * 0.60 + 80, size.y *
    0.60 + 30}, "Difficulte : 1"), &manage_difficulty);
...
body_t *init_settings_elem2(sfVector2u size, body_t *elem, int nb)
{
    ...
    (sfVector2f){size.x * 0.24, size.y * 0.35});
    Set_Scale(elem[8].sprite, 3, 3);
    Set_Scale(elem[9].sprite, 3, 3);
    Set_Scale(elem[10].sprite, 1.5, 1.5);
